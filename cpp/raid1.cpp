...
bool compare_first(tuple<string, int, int, int> *&lhs,
                   tuple<string, int, int, int> *&rhs) {
    if (get<1>(*lhs) == get<1>(*rhs)) return get<0>(*lhs) > get<0>(*rhs);
    return get<1>(*lhs) < get<1>(*rhs);
}
bool compare_second(tuple<string, int, int, int> *&lhs,
                    tuple<string, int, int, int> *&rhs) {
    if (get<2>(*lhs) == get<2>(*rhs)) return get<0>(*lhs) > get<0>(*rhs);
    return get<2>(*lhs) < get<2>(*rhs);
}
bool compare_third(tuple<string, int, int, int> *&lhs,
                   tuple<string, int, int, int> *&rhs) {
...
int main() {
    ...
    for (int i = 0; i < n; i++) {
        cin >> get<0>(mem[i]) >> get<1>(mem[i]) >> get<2>(mem[i]) >>
            get<3>(mem[i]);
